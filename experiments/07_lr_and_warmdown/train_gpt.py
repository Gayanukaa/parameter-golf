    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                step_size = row_max / 31.0
                if CastedLinear._noisy_qat:
                    noise = (torch.rand_like(w32) - 0.5) * step_size[:, None]
                    w_q = (w32 + noise).to(x.dtype)
                else:
                    scale = step_size.clamp_min(1.0 / 31.0)
...
def main() -> None:
    ...
    if master_process:
        ...
        logfile = f"logs/{args.run_id}.log"
        print(logfile)
    ...
    CastedLinear._noisy_qat = args.noisy_qat
    base_model = GPT(
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if max_wallclock_ms is None:
            ...
            if warmdown_start <= step < args.iterations:
                progress = 1.0 - (args.iterations - step) / max(args.warmdown_iters, 1)
            else:
                return 1.0
        else:
